...
import { Mahasiswa, Paginator } from '@/types/model';
...
defineProps<{
  data: Paginator<Mahasiswa>
}>()
const params = new URLSearchParams(window.location.search)

const formFilter = useForm({
  search: params.get('search') ?? '',
  show: params.get('show') == 'all' ? 'all' :

    parseInt(params.get('show') ?? '10'),
})
  ...
  <AppLayout>
    ...
    <div class="mt-6 bg-white border rounded-lg" v-if="data.data.length > 0">
      <Table>
        ...
        <TableBody>
          <TableRow v-for="mahasiswa in data.data" :key="mahasiswa.nim">
            ...
            <TableCell>{{ mahasiswa.angkatan }}</TableCell>
            <TableCell>
              <Popover @update:open="console.log(this)">
                ...
                <PopoverContent align="end" class="w-50 p-0">
                  <Button as-child class="w-full justify-start" variant="ghost" size="lg">
                    <Link :href="`/students/${mahasiswa.nim}/edit`">
                  ...
                  <PopoverClose class="w-full">
                    <Button class="!w-full justify-start" variant="ghost" size="lg"
                      @click="confirmDelete = mahasiswa.nim">
    ...
    <ConfirmModal :open="!!confirmDelete" @update-open="opened => confirmDelete = (!opened ? null : confirmDelete)"
      title="Hapus Data Mahasiswa" :text="`Anda yakin ingin menghapus data mahasiswa dengan NIM ${confirmDelete}?`"
      confirm-button="Hapus" @confirm="router.delete(`/students/${confirmDelete}`, {
        onSuccess: () => {
