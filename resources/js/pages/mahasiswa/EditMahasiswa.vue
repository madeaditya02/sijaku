...
import { MahasiswaFull } from '@/types/model';
...
const props = defineProps<{ mahasiswa: MahasiswaFull }>()
...
const form = useForm({
  ...
  nomor_telpon: props.mahasiswa.nomor_telpon,
  ...
  agama: props.mahasiswa.agama,
})
const tgl_lhr = new Date(props.mahasiswa.tanggal_lahir)

console.log(props.mahasiswa.tanggal_lahir);
const tanggal_lahir = ref(new CalendarDate(tgl_lhr.getFullYear(), tgl_lhr.getMonth() + 1, tgl_lhr.getDate())) as Ref<DateValue>
console.log(tanggal_lahir.value);
watch(tanggal_lahir, val => form.tanggal_lahir = moment(val?.toDate('GMT')).format('YYYY-MM-DD'))
</script>
<template>
  <AppLayout>
    <h1 class="text-3xl font-medium">Edit Mahasiswa</h1>
    <form
      @submit.prevent="form.transform(data => ({ ...data, prevNIM: mahasiswa.nim })).put(`/students/${mahasiswa.nim}`)"
      class="mt-8 grid grid-cols-6 gap-6">
      <div class="cols-span-6 sm:col-span-3 md:col-span-2">
      ...
      <div class="cols-span-6 sm:col-span-3 md:col-span-2 flex flex-col">
